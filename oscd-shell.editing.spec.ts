...
import { Edit, Insert } from '@omicronenergy/oscd-api';

import { waitForDialogState } from '@omicronenergy/oscd-test-utils/waitFors.js';
...
//     console.log(`Dialog open state: ${maybeDialogWithOpen.open}`);
//     // If the dialog has an open
...
describe('oscd-shell', () => {
  ...
  describe('with several documents loaded', () => {
    ...
    it('prevents the user from renaming the current doc to an already opened doc name', async () => {
      ...
      renameButton!.click();
      console.log(
        `fileNameUI.checkValidity = ${oscdShell.fileNameUI.checkValidity()}`,
      );
